...
function getPosition() {
  return new Promise<{ lat: number; lng: number; accuracy_m: number; position_at: string }>((resolve, reject) => {
    if (!navigator.geolocation) return reject(new Error("이 기기에서는 위치 인증을 사용할 수 없어요."))
    navigator.geolocation.getCurrentPosition(
      ...
      () => reject(new Error("위치 확인에 실패했어요. 브라우저의 위치 권한을 허용한 뒤 다시 시도해주세요.")),
  ...
  const submit = async () => {
    ...
    try {
      ...
      const data = await fetchWithAuth("/api/checkin", { method: "POST",
        ...
        // 예약은 선택 사항이다. 예약 링크로 들어온 경우에도 새 인증은
        // 예약/QR/직원 승인 없이 현재 위치만으로 처리한다.
        body: JSON.stringify({ place_id: ctx.place.id, community_id: picked, party_size: 1, ...position }),
    ...
    <main className="mx-auto min-h-screen max-w-md bg-white px-5 pb-12 pt-10">
      {loading ? <p role="status" className="py-20 text-center text-slate-500">불러오는 중...</p> : ctx ? <>
        ...
        {visit ? <section className="mt-7" aria-live="polite">
          ...
          <p className="text-sm text-slate-600">로그인한 뒤 매장 근처에서 위치 인증을 해주세요.</p>
          ...
          {ctx.reservation && <p className="mb-5 rounded-xl bg-slate-50 p-3 text-sm text-slate-600">{ctx.reservation.time} 예약이 있어요. 예약 없이도 매장에 도착하면 현재 위치로 인증할 수 있어요.</p>}
          <h2 className="font-bold">누구와 함께 왔나요?</h2>
          <p className="mt-2 text-xs leading-relaxed text-slate-500">예약 없이도 인증할 수 있어요. 크루 공동 방문은 멤버 2명 이상이 같은 날 2시간 안에 각자 현재 위치를 확인해야 해요.</p>
          <div className="mt-4 space-y-2">
            {ctx.crews.map(c => <button key={c.id} disabled={busy}
            ...
            <button disabled={busy} aria-pressed={picked === null} onClick={() => setPicked(null)}
          ...
          <button disabled={busy} onClick={submit} className="mt-6 w-full rounded-2xl bg-[#F5A623] py-3.5 font-bold text-white disabled:opacity-50">{busy ? "현재 위치를 확인하는 중..." : "현재 위치로 방문 인증"}</button>
          <p className="mt-3 text-center text-xs leading-relaxed text-slate-500">방문 인증은 매장 좌표와 현재 위치의 거리로 확인해요. 확인된 방문만 크루 기록과 도감에 쌓여요.</p>
        </section>}
