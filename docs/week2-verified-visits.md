...
1주차 기반 브랜치 위에 구현한 RDV-05/06. 1주차 PR #3이 아직 미병합이므로
이 PR의 base는 `codex/week1-foundation`이다. #3을 먼저 병합한 뒤 이 PR의 base를 main으로 변경한다.
운영 DB 변경, main 병합, Render/Vercel 운영 배포는 수행하지 않았다.
