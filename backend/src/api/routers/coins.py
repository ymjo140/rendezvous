...
def get_wallet(user: models.User = Depends(require_user), db: Session = Depends(get_db)):
    return service.get_wallet_info(db, user)

@router.post("/api/coins/charge")
def charge_coins(req: schemas.CoinChargeRequest, user: models.User = Depends(require_user), db: Session = Depends(get_db)):
    return service.charge_coins(db, user, req)

@router.post("/api/coins/map-loot")
def loot_coin(payload: dict, user: models.User = Depends(require_user), db: Session = Depends(get_db)):
