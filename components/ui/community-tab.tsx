    ...
    <div className="flex flex-col h-full bg-slate-50 font-['Pretendard']">
                  ...
                  <div className="flex justify-between items-start mb-2">
                    <div className="flex items-center gap-2">
                      <Avatar className="w-8 h-8">
                        <AvatarFallback className="bg-purple-50 text-[#7C3AED] font-bold">{m.author_name?.[0] || "U"}</AvatarFallback>
                      </Avatar>
      ...
      <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
        <DialogContent className="sm:max-w-md rounded-2xl">
          ...
          <DialogFooter>
            <Button onClick={handleCreate} className="w-full bg-[#7C3AED] h-12 rounded-xl text-base font-bold shadow-md hover:bg-[#6D28D9]">모임 만들고 일정에 추가하기</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
