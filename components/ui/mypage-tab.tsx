...
export function MyPageTab() {
  ...
  const handleEquip = async (item: AvatarItem) => {
      ...
      if (res.ok) {
          const data = await res.json();
          setPreviewEquipped(data.equipped);
          fetchMyInfo();
      }
  };
  ...
  const handleSubmitReview = async () => {
      ...
      try {
          ...
          if (res.ok) {
              alert("상세 리뷰가 등록되었습니다! AI가 취향을 학습했습니다.");
  ...
  const currentItems = activeTab === "shop" 
      ? shopItems.filter(i => i.category === activeCategory)
      : shopItems.filter(i => i.category === activeCategory && user?.avatar?.inventory?.includes(i.id));

  const renderAvatarLayered = (equippedState: Record<string, string | null>, height: number = 300) => {
      const width = height / 2; 
      const getUrl = (id: string | null | undefined) => {
          if (!id) return null;
          const item = shopItems.find(i => i.id === id);
          return item ? item.image_url : `/assets/avatar/${id}.png`; 
      };
      const bodyId = equippedState.body || "body_basic";
      const eyesId = equippedState.eyes || "eyes_normal";
      const browsId = equippedState.eyebrows || "brows_basic";
      
      const layers = [
          { id: 'body', url: getUrl(bodyId), z: 1 }, 
          { id: 'eyes', url: getUrl(eyesId), z: 2 }, 
          { id: 'eyebrows', url: getUrl(browsId), z: 3 },
          { id: 'bottom', url: getUrl(equippedState.bottom), z: 4 }, 
          { id: 'top', url: getUrl(equippedState.top), z: 5 },
          { id: 'shoes', url: getUrl(equippedState.shoes), z: 6, style: { bottom: 0, left: '10%', width: '80%', height: '15%', objectFit: 'contain' } }, 
          { id: 'hair', url: getUrl(equippedState.hair), z: 7 },
          { id: 'pet', url: getUrl(equippedState.pet), z: 8 }, 
      ];
      ...
      return (
          <div style={{ width: width, height: height, position: 'relative', margin: '0 auto' }}>
              ...
              {equippedState.pet && <img src={getUrl(equippedState.pet) || ''} alt="pet" style={{position: 'absolute', bottom: 0, right: -width*0.3, width: width*0.6, height: width*0.6, objectFit: 'contain', zIndex: 8}} className="animate-bounce" onError={(e) => e.currentTarget.style.display='none'} />}
          </div>
      );
  };
    ...
    <div className="space-y-4 h-full bg-slate-50 overflow-y-auto pb-20">
      ...
      <div className="px-4 space-y-4">
                    ...
                    {VISIT_HISTORY.map(place => (
                        <Card key={place.id} className="p-3 flex justify-between items-center bg-white shadow-sm">
                            <div>
                                ...
                                <div className="text-xs text-gray-500 mt-1">{place.tags.join(" · ")}</div>
                            </div>
                    ...
                    {user.reviews && user.reviews.length > 0 ? user.reviews.map((review: any) => (
                        <Card key={review.id} className="p-3 bg-white shadow-sm">
                            <div className="flex justify-between items-start mb-2">
                                <div className="font-bold text-sm">{review.place_name}</div>
                                <div className="text-yellow-500 text-xs font-bold">★ {review.rating.toFixed(1)}</div>
                            </div>
                            <div className="grid grid-cols-4 gap-1 text-[10px] text-gray-500 bg-gray-50 p-2 rounded mb-2">
                                <div>맛 {review.score_taste}</div>
                                <div>서비스 {review.score_service}</div>
                                <div>가격 {review.score_price}</div>
                                <div>분위기 {review.score_vibe}</div>
                            </div>
                            <p className="text-xs text-gray-700">{review.comment}</p>
                            <div className="mt-2 flex gap-1 flex-wrap">
                                {review.tags && review.tags.map((t:string, i:number) => <Badge key={i} variant="secondary" className="text-[10px] h-5 px-1">#{t}</Badge>)}
                            </div>
                        </Card>
                    )) : (
                        <div className="text-center text-gray-400 py-10 text-sm bg-gray-50 rounded-xl border border-dashed">작성한 리뷰가 없습니다.</div>
                    )}
                </TabsContent>
                    ...
                    {user.favorites && user.favorites.length > 0 ? (
                        user.favorites.map((fav: any, i: number) => (
                            <Card key={i} className="p-3 flex justify-between items-center bg-white shadow-sm">
                                ...
                                <Button size="sm" variant="ghost" className="h-8 text-xs text-gray-400">삭제</Button>
                            </Card>
                        ))
                    ) : (
                        <div className="text-center text-gray-400 py-10 text-sm bg-gray-50 rounded-xl border border-dashed">즐겨찾는 장소가 없습니다.</div>
                    )}
                </TabsContent>
            </Tabs>
        </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-1">
            ...
            <Button variant="ghost" className="w-full justify-start gap-3 h-12"><HelpCircle className="w-5 h-5" /> 고객 지원</Button>
      ...
      <Dialog open={isEditorOpen} onOpenChange={setIsEditorOpen}>
        <DialogContent className="sm:max-w-md h-[80vh] flex flex-col p-0 gap-0 overflow-hidden rounded-xl">
            <DialogHeader className="px-6 pt-4 pb-2 bg-white"><DialogTitle>아바타 꾸미기</DialogTitle><DialogDescription>나만의 아바타를 꾸미고 아이템을 구매하세요.</DialogDescription></DialogHeader>
                    ...
                    <div className="flex-1 overflow-y-auto p-4 bg-slate-50">
                        <div className="grid grid-cols-3 gap-3">
                            {currentItems.map(item => {
                                ...
                                return (
                                    <div key={item.id} className={`relative bg-white rounded-xl p-2 border-2 transition-all cursor-pointer hover:shadow-md ${isEquipped ? "border-indigo-500 bg-indigo-50" : "border-transparent shadow-sm"}`} onClick={() => activeTab === 'inventory' ? handleEquip(item) : null}>
                                        <div className="aspect-square bg-gray-100 rounded-lg mb-2 overflow-hidden flex items-center justify-center"><img src={item.image_url} alt={item.name} className="w-full h-full object-contain" onError={(e)=>e.currentTarget.src='https://via.placeholder.com/100'}/></div>
                                        <div className="text-xs font-bold truncate">{item.name}</div>
                                        {activeTab === 'shop' && !isOwned ? (<Button size="sm" className="w-full mt-2 h-7 text-[10px] bg-yellow-400 hover:bg-yellow-500 text-black font-bold" onClick={(e) => { e.stopPropagation(); handleBuy(item); }}>{item.price_coin} C</Button>) : (<div className="mt-2 text-[10px] text-center font-bold text-gray-400 py-1">{isEquipped ? "장착 중" : (isOwned ? "보유 중" : "")}</div>)}
                                    </div>
                                )
                            })}
                        </div>
      ...
      <Dialog open={isReviewOpen} onOpenChange={setIsReviewOpen}>
          <DialogContent className="sm:max-w-sm">
              <DialogHeader>
                  <DialogTitle>✍️ 상세 리뷰</DialogTitle>
                  <DialogDescription><span className="font-bold text-indigo-600">{targetPlace?.name}</span> 상세 평가</DialogDescription>
              </DialogHeader>
