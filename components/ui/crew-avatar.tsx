...
export function CrewAvatar({
  ...
  const selectedId: CrewAvatarId = avatarIdForMember(memberId, avatarId)
  ...
  return (
    <span className={boxClass} data-avatar-id={selectedId}>
      <Image
        src={avatar.src}
        alt={name ? name + " 아바타" : avatar.label}
        fill
        sizes={size === "lg" ? "96px" : size === "md" ? "64px" : "44px"}
        className={mode === "portrait" ? "object-cover object-top" : "object-contain object-bottom"}
        priority={size === "lg"}
      />
    </span>
  )
